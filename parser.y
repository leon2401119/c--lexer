...
%type <node> program global_decl_list global_decl function_decl block stmt_list decl_list decl var_decl type init_id_list init_id  stmt relop_expr relop_term relop_factor expr term factor var_ref if_tail else
...
function_decl	: type ID MK_LPAREN param_list MK_RPAREN MK_LBRACE block MK_RBRACE     
                ...
                | VOID ID MK_LPAREN param_list MK_RPAREN MK_LBRACE block MK_RBRACE      
                    {
                        /*TODO*/
                        /* why need seperate? can't we not merge VOID into type? */
...
dim_fn		: MK_LB expr_null MK_RB 
            ...
            | dim_fn MK_LB expr MK_RB
                {
                    /* Seems we cannot have simple id[expr] in parameter declaration? */
                    /* for example : void foo(int arr[10]) is not a valid sequence in this grammer */
...
var_decl	: type init_id_list MK_SEMICOLON 
                {
                    ...
                    /*AST_NODE* ids = Allocate(VARIABLE_DECL_LIST_NODE);
                    makeChild(ids, $2);
                    makeFamily($$, 2, $1, ids);*/
                    makeFamily($$, 2, $1, $2);
                }
            | ID id_list MK_SEMICOLON
                {
                    ...
                    /*AST_NODE* ids = Allocate(VARIABLE_DECL_LIST_NODE);
                    makeChild(ids, $2);
                    makeFamily($$, 2, makeIDNode($1, NORMAL_ID), ids);*/
...
init_id		: ID 
            ...
            | ID dim_decl
                {
                    /*TODO*/
                    //printf("got array decl\n");
                    $$ = makeChild(makeIDNode($1, ARRAY_ID),$2);
                }
            | ID OP_ASSIGN relop_expr 
                {
                    /*TODO*/
                    //printf("got var decl with init\n");
...
stmt		: MK_LBRACE block MK_RBRACE 
            ...
            /*TODO: | If Statement */
            | IF MK_LPAREN relop_expr MK_RPAREN stmt if_tail
            	{
...
if_tail		 : ELSE IF MK_LPAREN relop_expr MK_RPAREN stmt if_tail
                     {
			$$ = makeStmtNode(IF_STMT);
			makeFamily($$,3,$4,$6,$7);
                     }
...
factor		: MK_LPAREN relop_expr MK_RPAREN
            ...
            | OP_MINUS ID MK_LPAREN relop_expr_list MK_RPAREN
            	{
                    /*TODO*/
                    AST_NODE* node = makeStmtNode(FUNCTION_CALL_STMT);
                    makeFamily(node,2,makeIDNode($2, NORMAL_ID),$4);
                    $$ = makeExprNode(UNARY_OPERATION, UNARY_OP_NEGATIVE);
                    makeChild($$,node);
            	}
